...
public class FileNumbers {
    ...
    private static void createNumbersFile() {
        ...
        LOGGER.log(Level.FINE, "Начинаем записывать файл {0} ...", numbersPath.toAbsolutePath());
        try (BufferedWriter writer = Files.newBufferedWriter(numbersPath)) {
            for(int i = 0; i < 10; i++) {
                ...
                LOGGER.log(Level.FINE, "Строка {0} recorded ok ", i + 1);
            }
            LOGGER.log(Level.INFO, "Файл {0} создан!", numbersPath.toAbsolutePath());
        } catch (Exception ex) {
            System.out.println("Ошибка создания файла " + numbersPath.getFileName() + ": " + ex.getMessage());
            LOGGER.log(
                    Level.SEVERE,
                    String.format("Ошибка создания файла %s: %s", numbersPath.toAbsolutePath(), ex),
                    ex
            );
        }
    }
    ...
    private static void createOddNumbersFile() {
        ...
        LOGGER.log(Level.FINE, "Читаем с файла {0} ...", numbersPath.toAbsolutePath());
        LOGGER.log(Level.FINE, "Начинаем создание файла {0} ... ", oddNumbersPath.toAbsolutePath());
